...
from tiebenn.tools.nicetools import tt_theo_before_assoc
from tiebenn.tools.utm import from_latlon
from tiebenn.tools.visualization import plot_assoc


def phase_association(outputs: dict, data: dict, velmod: int, ev_lon: float, ev_lat: float, ev_time: UTCDateTime,
                      max_dist: float, plot: bool, mult_windows: bool, secs_before: int) -> dict:
    """
    ...
    config = {'use_amplitude': False, 'dims': ['x(km)', 'y(km)', 'z(km)'], 'vel': {'p': 5.6, 's': 3.9},
              'use_dbscan': False, 'dbscan_min_samples': 10, 'dbscan_eps': 3, 'oversample_factor': 10,
              'x(km)': (1e-3 * ev_east - 222.22, 1e-3 * ev_east + 222.22),
              'y(km)': (1e-3 * ev_nort - 222.22, 1e-3 * ev_nort + 222.22), 'z(km)': (0, 50), 'max_sigma11': 4.0,
              'max_sigma22': 3.0, 'max_sigma12': 3.0, 'method': 'BGMM'}

    config['bfgs_bounds'] = ((config['x(km)'][0] - 1, config['x(km)'][1] + 1),
                             (config['y(km)'][0] - 1, config['y(km)'][1] + 1), (0, config['z(km)'][1] + 1),
                             (None, None))
    ...
    z = []
    for sta in outputs:
        arrivals = taupmodel.get_travel_times(source_depth_in_km=5.,
                                              distance_in_degree=float(data[sta]['epic_distance']) / 111.11)
        for i in arrivals:
            if i.name.lower() == 's':
        ...
        for pick in outputs[sta].picks:
            ...
            if teo_verdict == 'pass':
                ...
                if sta not in stats:
                    ...
                    st_east, st_nort, st_zone_num, st_zone_lett = from_latlon(data[sta]['coords'][0],
                                                                              data[sta]['coords'][1])
    ...
    dfs = {'id': id_stations, 'id_stations': stats, 'longitude': longitude, 'latitude': latitude,
           'elevation(m)': elevation, 'x(km)': x, 'y(km)': y, 'z(km)': z}
    ...
    picks = picks[['timestamp', 'station', 'prob', 'type']].rename(
        columns={'timestamp': 'peak_time', 'prob': 'peak_value', 'type': 'phase'})
    stations = stations[['id_stations']].rename(columns={'id_stations': 'id'})
    events = events[['event_index', 'time', 'x(km)', 'y(km)', 'z(km)', 'num_picks']].rename(
        columns={'event_index': 'idx', 'x(km)': 'x', 'y(km)': 'y', 'z(km)': 'z', 'num_picks': 'picks'})

    event_idx_for_assig = [];
    pick_idx_for_assig = [];
    station_for_assig = [];
    phase_for_assig = [];
    peak_value_for_assig = [];
    peak_time_for_assig = []
    ...
    assignments = pd.DataFrame(
        data={'event_idx': event_idx_for_assig, 'pick_idx': pick_idx_for_assig, 'station': station_for_assig,
              'phase': phase_for_assig, 'peak_value': peak_value_for_assig, 'peak_time': peak_time_for_assig})
    ...
    logger.info('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
    logger.info('GaMMA: For a total of {} predicted picks'.format(picks_all))
    logger.info('of which {} are P-picks and {} are S-picks:'.format(picks_all_p, picks_all_s))
    logger.info('% {} event(s) was/were detected'.format(len(events)))

    for event in range(len(events)):
        logger.info('% for event {} :'.format(event + 1))
        ...
        logger.info('% {} picks were associated'.format(picks_event))
        logger.info('% of which {} are P-picks and {} are S-picks'.format(picks_event_p, picks_event_s))
    logger.info('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
