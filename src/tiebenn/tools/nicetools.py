...
from seisbench.util.annotations import Pick

from tiebenn.logger.logger_settings import logger


def decimate(phasepicks: list, data: dict, ev_lon: float, ev_lat: float, az_bin: int, dist_bin: int,
             max_bin: int) -> list:
    """
...
def tt_theo_before_assoc(ev_time: UTCDateTime, teo_p_time: float, teo_s_time: float, pick: Pick, tol_p: int,
                         tol_s: int) -> str:
    """
...
def create_input_for_phassoc(outputs: dict, seconds_before: list[int]) -> dict:
    """
...
def generate_csv(outputs: dict, data: dict, snr_data: dict, ev_time: str) -> None:
    """
    ...
    for stat in outputs[0]['station'].drop_duplicates():
        ...
        df = {'network': net, 'station': sta, 'station_lat': st_lat, 'station_lon': st_lon, 'station_elv': st_elv,
              'phase': phase, 'arrival_time': arrival_time, 'probability': probability, 'snr': snr}
...
def chan_comma(chanlist: list) -> str:
    """
...
def strmonth2num(strmonth: str) -> int:
    """
...
def get_snr(data: Stream, picks: pd.DataFrame, wlen: int) -> dict:
    """
    ...
    for p in picks['phase']:
        ...
        snr[pick['phase'].tolist()[0]] = round(10 * math.log10((np.percentile(sw1, 95) / np.percentile(nw1, 95)) ** 2),
                                               1)
    return snr


def station_density(epicenter: tuple, stations: list) -> np.float64:
    """
...
def calculate_azimuths(epicenter: tuple, stations: list) -> np.ndarray:
    """
...
def azimuthal_uniformity_index(azimuths: np.ndarray) -> np.float64:
    """
...
def azimuthal_gaps(event_lat: float, event_lon: float, station_coords: list) -> tuple[np.float64, np.float64]:
    """
    ...
    def calculate_azimuth(lat1: float, lon1: float, lat2: float, lon2: float) -> np.float64:
        """
    ...
    for i in range(len(azimuths_sorted)):
        ...
        if largest_gap > secondary_gap:
            ...
            secondary_pair = (reduced_azimuths[secondary_index],
                              reduced_azimuths[(secondary_index + 1) % len(reduced_azimuths)])

    return primary_gap, secondary_gap


def normalize(param: np.float64, mode: str, lb: float, ub: float) -> np.float64:
    """
...
def calculate_lqs(loc_file: str, sta_file: str) -> pd.DataFrame:
    """
    ...
    """
    logger.info('Generating radarplot from LQS value...')
    ...
    with open(sta_file, 'r') as f:
        for line in f:
            if line.split()[1] in st4loc:
                st_lat = float(line.split()[3]);
                st_lon = float(line.split()[4])
    ...
    weights = {'sta_den': 0.125, 'azgap': 0.05, 'sec_azgap': 0.1, 'rms': 0.1, 'near_sta': 0.1, 'det_cov': 0.35,
               'aui': 0.125, 'npicks': 0.05}

    LQS = weights['sta_den'] * norm_dens + weights['azgap'] * (1 - norm_azgap) + weights['sec_azgap'] * (
            1 - norm_sazgap) + weights['rms'] * (1 - norm_rms) + weights['near_sta'] * (1 - norm_nsta) + weights[
              'det_cov'] * (1 - norm_detcov) + weights['npicks'] * norm_npicks + weights['aui'] * (1 - norm_aui)

    norm_params = {'norm.det.cov': norm_detcov, 'norm.sta.den': norm_dens, 'norm.aui': norm_aui,
                   'norm.azgap': norm_azgap, 'norm.sec.azgap': norm_sazgap, 'norm.near.sta': norm_nsta,
                   'norm.rms': norm_rms, 'norm.npicks': norm_npicks}
