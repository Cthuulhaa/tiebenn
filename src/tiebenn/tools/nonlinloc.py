...
from tiebenn.tools.velocity_models import velmods
from tiebenn.tools.visualization import epic_sta_plot
...
def inp_files_nlloc_sb(ev_lon: float, ev_lat: float, ev_time: str, data: dict, nll3d: bool, velmod: int,
                       min_detections: int = 3, verbosity: int = 0) -> None:
    """
    ...
    if not len(glob.glob('*_tiebenn_loc')):
        logger.info('No P- and S-wave arrival times were detected for this event. Stop.')
        return

    logger.info('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
    logger.info('---------------------------------------------------------------')
    logger.info('Preparing input files for hypocentre location with NonLinLoc...')
    logger.info('---------------------------------------------------------------')
    logger.info('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
        ...
        if velmod not in [12, 13, 17]:
            try:
                for rem in glob.glob('*_tiebenn_loc/nll_control*'):
                    os.remove(rem)
                os.remove(glob.glob('*_tiebenn_loc/obs_ttimes.obs')[0]);
                os.remove(glob.glob('*_tiebenn_loc/station_coordinates.txt')[0])
            except:
                pass
    ...
    with open(f"{glob.glob('*_tiebenn_loc/')[0]}station_coordinates.txt", 'w') as file1:
        for stations in range(len(glob.glob('*_tiebenn_loc/csv_picks/*.csv'))):
            ...
            if velmod == 17:
                lat_ = data[station]['coords'][0];
                lon_ = data[station]['coords'][1]
    ...
    def prob2uncer(x: np.float64) -> np.float64:
        """
    ...
    with open(f"{glob.glob('*_tiebenn_loc/')[0]}obs_ttimes.obs", 'w') as file2:
        for obs in glob.glob('*_tiebenn_loc/csv_picks/*.csv'):
            ...
            for pick in range(len(dataframe)):
                ...
                second = UTCDateTime(dataframe.arrival_time[pick]).second + 1e-6 * UTCDateTime(
                    dataframe.arrival_time[pick]).microsecond
    ...
    with open(f"{glob.glob('*_tiebenn_loc/')[0]}nlloc_control.in", 'w') as file3:
        ...
        if velmod != 13:
            file3.write(
                f"TRANS   LAMBERT   Clarke-1880   {str(ev_lat)}   {str(ev_lon)}   {str(ev_lat - 1)}   {str(ev_lat + 1)}   {str(0.0)}\n")
        else:
        ...
        if velmod not in [13, 17]:
            file3.write(
                f"GTFILES   {glob.glob('*_tiebenn_loc/')[0]}model_layer   {glob.glob('*_tiebenn_loc/')[0]}time_layer   P\n")
        elif velmod == 13:
        ...
        file3.write(
            f"LOCFILES   {glob.glob('*_tiebenn_loc/')[0]}obs_ttimes.obs   NLLOC_OBS   {glob.glob('*_tiebenn_loc/')[0]}time_layer   {glob.glob('*_tiebenn_loc/')[0]}loc_eqdatetime\n")
    ...
    if velmod != 17:
        with open(f"{glob.glob('*_tiebenn_loc/')[0]}nlloc_control_s.in", 'w') as file4:
            ...
            if velmod != 13:
                file4.write(
                    f"GTFILES   {glob.glob('*_tiebenn_loc/')[0]}model_layer   {glob.glob('*_tiebenn_loc/')[0]}time_layer   S\n")
            else:
                model_path = extract_3d_velmod(model_name='weg', workdir='temp_velmodfiles')
...
def pynlloc(control_file: str, control_file_s: str, data, velmod: int, nll3d: bool, plots: bool) -> tuple[
    float, float, dict]:
    """
    ...
    def Vel2Grid(control_file: str) -> None:
        """
        ...
        #print("Vel2Grid in PATH:", shutil.which("Vel2Grid"))
        #print("Control file exists:", os.path.exists(control_file))

        #control_file = '/home/zakharova/PycharmProjects/tiebenn/test/2024-12-24T01:55:04.000000Z_tiebenn_loc/nlloc_control.in'

        # process = subprocess.Popen(['/home/zakharova/PycharmProjects/tiebenn/Vel2Grid', control_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

        process = subprocess.Popen(['Vel2Grid', control_file],
    ...
    def Vel2Grid3D(control_file: str) -> None:
        """
    ...
    def Grid2Time(control_file: str) -> None:
        """
    ...
    def NLLoc(control_file: str) -> None:
        """
    ...
    def Grid2GMT(control_file: str, gridloc: str, gmt_dir: str, plot_type1: str, plot_type2: str) -> None:
        """
        ...
        process = subprocess.Popen(['Grid2GMT', control_file, gridloc, gmt_dir, plot_type1, plot_type2],
    ...
    if not glob.glob(f"{glob.glob('*_tiebenn_loc/')[0]}nlloc_control.in"):
        logger.info('NLL: Stop.')
        return

    logger.info('NonLinLoc: Locating event...')

    try:
        if not nll3d:
            logger.info('NLL: Producing velocity/slowness model from velocity description...')
            if velmod not in [12, 13]:
        ...
        if not nll3d:
            logger.info('NLL: Calculating P-wave travel times...')
        else:
            logger.info('NLL3D: Calculating P-wave travel times...')
        ...
        if not nll3d:
            logger.info('NLL: Calculating S-wave travel times...')
        else:
            logger.info('NLL3D: Calculating S-wave travel times...')
        ...
        if not nll3d:
            logger.info('NLL: Depth estimation...')
        else:
            logger.info('NLL3D: Depth estimation...')
        NLLoc(control_file)
        ...
        with open(glob.glob('*_tiebenn_loc/*.hyp')[0], 'r') as f:
            lines = f.readlines()
            location_check, sta_gap, event_final_results = get_final_results(lines, reject, location, nll3d)
    ...
    return sta_gap, sta_nearest, event_final_results


def get_final_results(lines: List[str], reject: str, location: str, nll3d: bool) -> None | tuple[
    None, None, dict[Any, Any]] | tuple[bool, float, dict[Any, Any]]:
    event_final_results: dict[Any, Any] = {}
    for line in lines:
        if line.find(reject) != -1:
            logger.info('NonLinLoc: location was', reject)
            return None, None, event_final_results
        else:
            if line.find(location) != -1:
                location_check = True
                logger.info('##############################################################')
                if not nll3d:
                    logger.info('NonLinLoc: {}'.format(location))
                else:
                    logger.info('NonLinLoc3D: {}'.format(location))
                for out_line in lines:
                    if out_line.find('GEOGRAPHIC') != -1:
                        ...
                        event_date = "-".join([out_line.split()[4], out_line.split()[3], out_line.split()[2]])
                        event_time = ":".join([out_line.split()[5], out_line.split()[6], out_line.split()[7]])
                        event_datetime = "_".join([event_date, event_time])
                        event_depth = out_line.split()[13]

                        event_final_results["datetime"] = event_datetime
                        event_final_results["latitude"] = event_latitude
                        event_final_results["longitude"] = event_longitude
                        event_final_results["depth"] = event_depth

                        logger.info('Origin time:')
                        logger.info("           {}".format(event_datetime))
                        logger.info('           {}: {}'.format(out_line.split()[8], event_latitude))
                        logger.info('           {}: {}'.format(out_line.split()[10], event_longitude))
                        logger.info('           {}: {} km'.format(out_line.split()[12], event_depth))

                        logger.info('--------------------------------------------------------------')
                        logger.info('Location quality:')

                    if out_line.find('STATISTICS') != -1:
                        logger.info('           Ellipsoid semi-major axis: {}'.format(out_line.split()[-1]))

                    if out_line.find('QUALITY') != -1:
                        logger.info('           {}: {}'.format(out_line.split()[7], out_line.split()[8]))
                        logger.info('           Number of phases: {}'.format(out_line.split()[10]))
                        logger.info('           {}: {}'.format(out_line.split()[11], out_line.split()[12]))
                        logger.info('           Distance from hypocenter to nearest station: {} km'.format(
                            out_line.split()[14]))

                        sta_gap = float(out_line.split()[12])

                        # RMS = root square mean
                        estimation_rms = out_line.split()[8]
                        event_final_results["RMS"] = estimation_rms

                logger.info('##############################################################')

                return location_check, sta_gap, event_final_results


def create3dgrid(ev_lon: float, ev_lat: float, velmod: int) -> None:
    """
    ...
    """
    logger.info('Creating 3D NLLGrids...')
    ...
    v33 = {};
    v33['vp'] = np.asarray(vp_values);
    v33['vs'] = np.asarray(vs_values)
    ...
    v32 = {};
    v32['vp'] = np.asarray(vp_values);
    v32['vs'] = np.asarray(vs_values)
    ...
    v34 = {};
    v34['vp'] = np.asarray(vp_values);
    v34['vs'] = np.asarray(vs_values)
    ...
    v22 = {};
    v22['vp'] = np.asarray(vp_values);
    v22['vs'] = np.asarray(vs_values)
    ...
    v23 = {};
    v23['vp'] = np.asarray(vp_values);
    v23['vs'] = np.asarray(vs_values)
    ...
    v24 = {};
    v24['vp'] = np.asarray(vp_values);
    v24['vs'] = np.asarray(vs_values)
    ...
    v42 = {};
    v42['vp'] = np.asarray(vp_values);
    v42['vs'] = np.asarray(vs_values)
    ...
    v43 = {};
    v43['vp'] = np.asarray(vp_values);
    v43['vs'] = np.asarray(vs_values)
    ...
    v44 = {};
    v44['vp'] = np.asarray(vp_values);
    v44['vs'] = np.asarray(vs_values)
    ...
    v11 = {};
    v11['vp'] = np.asarray(vp_values);
    v11['vs'] = np.asarray(vs_values)
    ...
    v12 = {};
    v12['vp'] = np.asarray(vp_values);
    v12['vs'] = np.asarray(vs_values)
    ...
    v13 = {};
    v13['vp'] = np.asarray(vp_values);
    v13['vs'] = np.asarray(vs_values)
    ...
    v14 = {};
    v14['vp'] = np.asarray(vp_values);
    v14['vs'] = np.asarray(vs_values)
    ...
    v15 = {};
    v15['vp'] = np.asarray(vp_values);
    v15['vs'] = np.asarray(vs_values)
    ...
    v21 = {};
    v21['vp'] = np.asarray(vp_values);
    v21['vs'] = np.asarray(vs_values)
    ...
    v25 = {};
    v25['vp'] = np.asarray(vp_values);
    v25['vs'] = np.asarray(vs_values)
    ...
    v31 = {};
    v31['vp'] = np.asarray(vp_values);
    v31['vs'] = np.asarray(vs_values)
    ...
    v35 = {};
    v35['vp'] = np.asarray(vp_values);
    v35['vs'] = np.asarray(vs_values)
    ...
    v41 = {};
    v41['vp'] = np.asarray(vp_values);
    v41['vs'] = np.asarray(vs_values)
    ...
    v45 = {};
    v45['vp'] = np.asarray(vp_values);
    v45['vs'] = np.asarray(vs_values)
    ...
    v51 = {};
    v51['vp'] = np.asarray(vp_values);
    v51['vs'] = np.asarray(vs_values)
    ...
    v52 = {};
    v52['vp'] = np.asarray(vp_values);
    v52['vs'] = np.asarray(vs_values)
    ...
    v53 = {};
    v53['vp'] = np.asarray(vp_values);
    v53['vs'] = np.asarray(vs_values)
    ...
    v54 = {};
    v54['vp'] = np.asarray(vp_values);
    v54['vs'] = np.asarray(vs_values)
    ...
    v55 = {};
    v55['vp'] = np.asarray(vp_values);
    v55['vs'] = np.asarray(vs_values)
    ...
    try:
        ...
        logger.warning('Problem moving grid to *_tiebenn_loc directory.')
        pass
    return
