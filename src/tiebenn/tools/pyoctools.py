...
def phase_association(outputs: dict, data: dict, velmod: int, ev_lon: float, ev_lat: float, ev_time: UTCDateTime,
                      max_dist: float, plot: bool, mult_windows: bool, secs_before: int | list[int]):
    """
    ...
    if len(station) <= 3:
        logger.warning('PyOcto: not enough picks passed the comparison with theoretical arrival times! Skipping.')
        return
    ...
    if outputs_s <= 3:
        associator = pyocto.OctoAssociator.from_area(lat=(ev_lat - 1.3, ev_lat + 1.3), lon=(ev_lon - 1.3, ev_lon + 1.3),
                                                     zlim=(0, 100), time_before=300, velocity_model=velocity_model,
                                                     n_p_picks=2, n_s_picks=2, n_picks=3, n_p_and_s_picks=1,
                                                     pick_match_tolerance=2.5)
    elif outputs_s > 3 and outputs_s < 6:
        associator = pyocto.OctoAssociator.from_area(lat=(ev_lat - 1.3, ev_lat + 1.3), lon=(ev_lon - 1.3, ev_lon + 1.3),
                                                     zlim=(0, 100), time_before=400, velocity_model=velocity_model,
                                                     n_p_picks=2, n_s_picks=2, n_picks=5, n_p_and_s_picks=2,
                                                     pick_match_tolerance=2.5)
    else:
        associator = pyocto.OctoAssociator.from_area(lat=(ev_lat - 1.3, ev_lat + 1.3), lon=(ev_lon - 1.3, ev_lon + 1.3),
                                                     zlim=(0, 100), time_before=400, velocity_model=velocity_model,
                                                     n_p_picks=3, n_s_picks=3, n_picks=6, n_p_and_s_picks=3,
                                                     pick_match_tolerance=2.5)
    ...
    try:
        ...
        logger.warning('PyOcto: No arrivals were associated to this event %s seconds before the event.' % secs_before)
        return
    ...
    logger.info('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
    logger.info('PyOcto: Phase association with waveforms starting %s seconds before event time' % str(secs_before))
    logger.info('------------------------------------------------------------------------------')
    logger.info('For a total of %s predicted picks which are comparable to theoretical picks' % str(picks_all))
    ...
    for event in range(len(events)):
        ...
        logger.info('% {} picks were associated'.format(picks_event_s))
