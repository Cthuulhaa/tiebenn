...
def process_client(cl: str, ev_lon: float, ev_lat: float, start_time: UTCDateTime, end_time: UTCDateTime,
                   channel_list: list = [], filter_network: list = [], filter_station: list = []) -> dict:
    """
    ...
    client_station_list = {}
    try:
        inventory = client_fdsn(cl).get_stations(minlatitude=min_lat, maxlatitude=max_lat, minlongitude=min_lon,
                                                 maxlongitude=max_lon, starttime=UTCDateTime(start_time),
                                                 endtime=UTCDateTime(end_time), endafter=UTCDateTime(start_time),
                                                 level='channel')
                ...
                for st in ev:
                    ...
                    if station not in filter_station:
                        ...
                        if new_chan and (station not in client_station_list):
                            client_station_list[str(station)] = {'network': net, 'channels': list(set(new_chan)),
                                                                 'coords': [lat, lon, elv], 'client': cl}
...
def make_station_list(client_list: list, ev_lon: float, ev_lat: float, start_time: UTCDateTime, end_time: UTCDateTime,
                      channel_list: list = [], filter_network: list = [], filter_station: list = []) -> dict:
    """
