...
def velmods(model: int, ev_lon: float, ev_lat: float):
    """
    ...
    try:
        ...
        if model == 6:
            ...
            for i in crust1.get_point(ev_lat, ev_lon):
                velmod.append(
                    f"LAYER   {str(-crust1.get_point(ev_lat, ev_lon)[i][4])}   {str(crust1.get_point(ev_lat, ev_lon)[i][0])}   0.0   {str(crust1.get_point(ev_lat, ev_lon)[i][1])}   0.0   {str(crust1.get_point(ev_lat, ev_lon)[i][2])}    0.0")
...
class crustModel:
    ...
    def __init__(self):
        ...
        self.layer_names = ['water', 'ice', 'upper_sediments', 'middle_sediments', 'lower_sediments', 'upper_crust',
                            'middle_crust', 'lower_crust', 'mantle']

    def _get_index(self, lat: float, lon: float) -> tuple[int, int]:
        """
...
def select_velmod(ev_lat: float, ev_lon: float) -> int:
    """
    ...
    """
    if (ev_lon > 9.7 and ev_lon <= 13 and ev_lat >= 47 and ev_lat <= 48) or (
            ev_lon > 13 and ev_lon <= 15 and ev_lat >= 47 and ev_lat <= 49):
        velmod = 14
    ...
    logger.info('Selected velocity model for the input latitude and longitude: %s' % str(velmod))
