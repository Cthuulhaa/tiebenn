...
def plotpicks_sb_mw(data: dict, streams: Stream, starttime: UTCDateTime, predictions: dict, picks: dict,
                    picks_final: pd.DataFrame) -> None:
    """
    ...
    try:
        ...
        for ch in range(len(streams)):
            axis[ax0].plot(streams[ch].times() - secs_bef, streams[ch].data / np.amax(streams[ch].data), 'k',
                           linewidth=1)
            ...
            for seconds in picks[sta]:
                for pick in picks[sta][seconds].picks:
                    ...
                    axis[ax0].vlines(pick_time, min(streams[ch].data / np.amax(streams[ch].data)),
                                     max(streams[ch].data / np.amax(streams[ch].data)), linestyles='solid',
                                     colors='lightgrey', linewidth=1)

            pick_pl = picks_final[picks_final['station'] == sta]
            for pick in range(len(pick_pl)):
                pick_time = UTCDateTime(str(pick_pl['peak_time'].to_list()[pick])) - streams[
                    ch].stats.starttime - secs_bef
                if pick_pl['phase'].to_list()[pick] == 'P':
                    axis[ax0].vlines(pick_time, min(streams[ch].data / np.amax(streams[ch].data)),
                                     max(streams[ch].data / np.amax(streams[ch].data)), linestyles='solid', colors='c',
                                     label='P-pick')
                if pick_pl['phase'].to_list()[pick] == 'S':
                    axis[ax0].vlines(pick_time, min(streams[ch].data / np.amax(streams[ch].data)),
                                     max(streams[ch].data / np.amax(streams[ch].data)), linestyles='solid', colors='r',
                                     label='S-pick')
        ...
        for seconds in predictions[sta]:
            for preds in range(len(predictions[sta][seconds])):
                if predictions[sta][seconds][preds].stats.channel.split('_')[-1] != 'N':
                    ...
                    axis[ax0].plot(predictions[sta][seconds][preds].times() + offset - secs_bef,
                                   predictions[sta][seconds][preds].data, color='lightgrey', linewidth=1)

        for pick in range(len(pick_pl)):
            ind_max = pick_pl['index'].to_list()[pick]
            for pred in range(len(predictions[sta][ind_max])):
                if predictions[sta][str(ind_max)][pred].stats.channel.split('_')[-1] == pick_pl['phase'].to_list()[
                    pick]:
                    offset = predictions[sta][str(ind_max)][pred].stats.starttime - streams[ch].stats.starttime
                    axis[ax0].plot(predictions[sta][str(ind_max)][pred].times() + offset - secs_bef,
                                   predictions[sta][str(ind_max)][pred].data,
                                   label=predictions[sta][str(ind_max)][pred].stats.channel.split('_')[-1])
        ...
        logger.info(f'No data to plot for station {sta}')
        pass
...
def plotpicks_sb(data: dict, streams: Stream, starttime: UTCDateTime, predictions: dict, picks: dict) -> None:
    """
    ...
    try:
        ...
        for ax, trace in zip(axes[:-1], streams):
            ...
            for pick in picks[sta].picks:
                if pick.phase == 'P':
                    ax.vlines(pick.peak_time - trace.stats.starttime, np.min(trace.data / np.amax(trace.data)),
                              np.max(trace.data / np.amax(trace.data)), linestyles='solid', colors='c', label='P-pick')
                    ax.legend(loc='upper right')
                elif pick.phase == 'S':
                    ax.vlines(pick.peak_time - trace.stats.starttime, np.min(trace.data / np.amax(trace.data)),
                              np.max(trace.data / np.amax(trace.data)), linestyles='solid', colors='r', label='S-pick')
                    ax.legend(loc='upper right')
        ...
        for preds in range(len(predictions[sta])):
            if predictions[sta][preds].stats.channel.split('_')[-1] != 'N':
                ...
                last_ax.plot(predictions[sta][preds].times() + offset, predictions[sta][preds].data,
                             label=predictions[sta][preds].stats.channel.split('_')[-1])
        ...
        logger.error('No data to plot for station', sta, 'Error:', e)
        return
    ...
    try:
        ...
        logger.error('Error saving figure for station', sta, 'Error:', e)

    return


def plot_assoc(ev_time: UTCDateTime, data: dict, stations: pd.DataFrame, picks: pd.DataFrame, events: pd.DataFrame,
               merged: pd.DataFrame, mult_windows: bool, secs_before: int) -> None:
    """
    ...
    for ev in range(len(events)):
        ...
        for station in stations['id']:
            ...
            ax.text(epic_dist, 0, station, size=10, rotation=40, ha='center', va='center',
                    bbox=dict(boxstyle='square', ec=(0, 0, 0), fc=(1, 0.9, 0.9)))
...
def epic_sta_plot() -> None:
    ...
    ax.plot(ev_lon, ev_lat, marker='s', markersize=8, markeredgecolor='black', markerfacecolor='darkred',
            transform=ccrs.PlateCarree(), label='Epicenter')

    ax.scatter(df['longitude'], df['latitude'], marker='^', edgecolor='black', s=50, color='deepskyblue',
               transform=ccrs.PlateCarree())

    for _, row in df.iterrows():
        if row['longitude'] > min_lon and row['longitude'] < max_lon and row['latitude'] > min_lat and row[
            'latitude'] < max_lat:
            ax.text(row['longitude'], row['latitude'], row['station'], fontsize=10, fontfamily='monospace',
                    fontweight='bold', transform=ccrs.PlateCarree(), horizontalalignment='left',
...
def plot_picks4loc(data: dict, streams: dict) -> None:
    """
    ...
    for st in open(stations_file, 'r'):
        ...
        ax.plot(streams[station][0].times() - time_diff,
                (streams[station][0].data / np.amax(streams[station][0].data)) + distance, 'k', linewidth=1)
        ...
        for obs in open(observations_file, 'r'):
            if obs.split()[0] == station:
                ...
                if obs.split()[4] == 'P':
                    ax.vlines(pick_time - streams[station][0].stats.starttime - time_diff,
                              min(streams[station][0].data / np.amax(streams[station][0].data)) + distance,
                              max(streams[station][0].data / np.amax(streams[station][0].data)) + distance,
                              linestyles='solid', colors='c', label='P-pick')
                if obs.split()[4] == 'S':
                    ax.vlines(pick_time - streams[station][0].stats.starttime - time_diff,
                              min(streams[station][0].data / np.amax(streams[station][0].data)) + distance,
                              max(streams[station][0].data / np.amax(streams[station][0].data)) + distance,
                              linestyles='solid', colors='r', label='S-pick')
...
def plot_hypoc_confidence_ellipsoid() -> None:
    """
    ...
    for lon, lat, name in stations:
        if float(lon) > region_xy[0] and float(lon) < region_xy[1] and float(lat) > region_xy[2] and float(lat) < \
                region_xy[3]:
...
def radar_factory(num_vars: int) -> np.ndarray:
    ...
    class RadarTransform(PolarAxes.PolarTransform):
        ...
        def transform_path_non_affine(self, path) -> matplotlib.path.Path:
    ...
    class RadarAxes(PolarAxes):
        ...
        def fill(self, *args, closed: bool = True, **kwargs) -> list:
        ...
        def _gen_axes_patch(self) -> matplotlib.patches.RegularPolygon:
...
def radarplot(event: pd.DataFrame) -> None:
    """
    ...
    parameters = [1 - event['norm.det.cov'].tolist()[0], event['norm.sta.den'].tolist()[0],
                  1 - event['norm.aui'].tolist()[0], 1 - event['norm.azgap'].tolist()[0],
                  1 - event['norm.sec.azgap'].tolist()[0], 1 - event['norm.near.sta'].tolist()[0],
                  1 - event['norm.rms'].tolist()[0], event['norm.npicks'].tolist()[0]]
    ...
    ax.text(0.5, -0.12, f"LQS = {event['LQS'].tolist()[0]:.2f}", size=20, transform=ax.transAxes, ha='center',
    ...
    labels = [r'1 - det(Cov[$\bf{X}$])', 'Sta.Dens.', '1 - Az.Unif.', '1 - Az.Gap', '1 - Sec.Az.Gap', '1 - Near.Sta',
              '1 - RMS', 'N.Picks']
    ...
    for angle, label, value, rotation in zip(theta, labels, parameters, rotations):
        ...
        ax.text(angle, r_value, f"{value:.2f}", size=14, rotation=rotation, rotation_mode='anchor', ha='center',
